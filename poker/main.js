const compareHands = (handF, handS) => {
...
class PokerHand {
  ...
  evaluateHandComponents() {
    ...
    this.handComponents.pairs.sort(
      (a, b) =>
        PokerHand.VALUE_ORDER.indexOf(b) - PokerHand.VALUE_ORDER.indexOf(a),
    );
  }
  ...
  compareTieBreaker(otherHand) {
    ...
    if (rank === 5) {
      if (this.handComponents.triplet > otherHand.handComponents.triplet)
        return 1;
      if (this.handComponents.triplet < otherHand.handComponents.triplet)
        return -1;
      if (this.handComponents.pairs[0] > otherHand.handComponents.pairs[0])
        return 1;
      if (this.handComponents.pairs[0] < otherHand.handComponents.pairs[0])
        return -1;
    }
    ...
    if (rank === 3) {
      ...
      if (this.handComponents.pairs[0] < otherHand.handComponents.pairs[0])
        return -1;
      if (this.handComponents.pairs[1] > otherHand.handComponents.pairs[1])
        return 1;
      if (this.handComponents.pairs[1] < otherHand.handComponents.pairs[1])
        return -1;
    }
